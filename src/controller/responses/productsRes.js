...
export const getAllProducts = async (req, res) => {
    const { page = 1, limit = 6, sort=-5, filter='0_-1_1' } = req.query;
    try {
...
export const getAllProductsByCategory = async (req, res) => {
    ...
    try {
      ...
      const products = await Product.find({ category: categoryId }, filterQuery).skip(startIndex).limit(limit)
          .sort({
            [sortWay]: (sort >= 0 ? 1 : -1 ) 
          });
...
export const getAllProductsByCategoryAndSubcategory = async (req, res) => {
    ...
    try {
      ...
      const products = await Product.find({ category: categoryId, subcategory: subcategoryId }, filterQuery).skip(startIndex).limit(limit).sort({
        [sortWay]: (sort >= 0 ? 1 : -1 ) 
      });
...
export const getAllProductsBySubcategory = async (req, res) => {
  ...
  try {
    ...
    const products = await Product.find({ subcategory: subcategoryId }, filterQuery).skip(startIndex).limit(limit).sort({
      [sortWay]: (sort >= 0 ? 1 : -1 ) 
    });
